...
export default function StaffLogin() {
  ...
  const handlePasscodeLogin = async (pin) => {
    ...
    setIsLoading(true);
    try {
      const result = await loginStaff(pin, restaurantId);
      ...
      <div className="w-full max-w-md">
        <div className="rounded-2xl bg-card p-8 shadow-lg border border-stroke">
          <div className="mb-6 text-center">
            ...
            <p className="mt-1 text-sm font-medium text-green-600">{restaurantName}</p>
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              className="w-full rounded-lg bg-green-600 py-3 text-sm font-medium text-white transition hover:bg-green-700 disabled:opacity-60 cursor-pointer"
            >
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm">
            ...
            <Link to={`/manager-login/${restaurantId}`} className="text-primary hover:underline">Manager Login</Link>
          </div>
        </div>
      </div>
    </div>
